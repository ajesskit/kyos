  ...
  if (hasExistingClaudeSetup && !apply) {
    return {
      ...
      warnings: [
        ...
        "Review the proposal, then run 'npx agentic-framework-init --apply' to apply only safe create/update actions.",
      ],
    };
  }
...
function runDoctor({ cwd }) {
  ...
  if (!readJsonIfExists(resolveRepoPath(cwd, USER_CONFIG_FILE))) {
    warnings.push(`${USER_CONFIG_FILE} is missing. Run 'npx agentic-framework-init' to create it.`);
  }
